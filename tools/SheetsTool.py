...
class SheetsTool():
    ...
    def connect_to_google(self) -> gspread.Spreadsheet:
        ...
        try:
            ...
            self.logger.error(
                f'Error connecting to Google Sheets -> {e}', exc_info=True)
            raise
    ...
    def update_sheets(self) -> None:
        ...
        try:
            ...
            # posted date
            worksheet.update_cell(self.task_id + 4, 7,
                                  self.d.strftime('%d-%b-%Y'))
            # posted time
            tz = datetime.timezone(datetime.timedelta(hours=0), name='UTC')
            worksheet.update_cell(self.task_id + 4, 8,
                                  self.d.astimezone(tz).strftime('%H:%M'))
            # posted by
    ...
    def update_master_sheet(self) -> None:
        ...
        try:
            ...
            worksheet.update_cell(
                self.task_id + 4, Constants.TEAMS_SHEETS_COLUMN_DICT.get(self.team), 'Complete')
            self.logger.info('update_master_sheet finished.')
        except Exception as e:
            self.logger.error(
                f'Error updating master sheet -> {e}', exc_info=True)
            raise
    ...
    def add_purple(self, player: str) -> None:
        ...
        try:
            ...
            book = self.connect_to_google()
            worksheet = book.worksheet(self.team)
            values = ['', '', '', '', self.purple, self.d.date().strftime(
                '%d-%b-%Y'), self.d.time().strftime('%H:%M'), player]
